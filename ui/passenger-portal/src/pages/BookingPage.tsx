...
export const BookingPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    for (const passenger of passengers) {
      if (!passenger.lastName || !passenger.firstName || !passenger.birthDate || !passenger.documentNumber) {
    ...
    try {
      ...
      if (paymentMethod === 'cash') {
        navigate(`/confirmation?ticketIds=${result.tickets.map(t => t.id).join(',')}`);
      } else {
        navigate(`/payment?paymentId=${result.payment.id}`);
      }
      reset();
    } catch (err: any) {
      setError(err.response?.data?.error || 'Ошибка при оформлении билета');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className={styles.container}>
      <div className={styles.content}>
        ...
        <div className={styles.tripInfo}>
          ...
          <Text>
            {selectedTrip.route?.fromStation?.name} → {selectedTrip.route?.toStation?.name}
          </Text>
          <Text>
            {formatDate(selectedTrip.departureTime)} в {formatTime(selectedTrip.departureTime)}
          </Text>
              ...
              <div className={styles.form}>
                ...
                {passenger.documentType === 'passport' && (
                  <Input
                    ...
                    onChange={(e) => handlePassengerChange(index, 'documentSeries', e.target.value)}
                  />
                )}
                <Input
                ...
                <Select
                  ...
                  onChange={(e) => {
                    ...
                    handlePassengerChange(index, 'discount', discounts[type || 'none'] || 0);
                  }}
                >
        ...
        <div className={styles.section}>
          ...
          <RadioGroup
            value={paymentMethod}
            onChange={(_, data) => setPaymentMethod(data.value as any)}
          >
        ...
        <div className={styles.summary}>
          ...
          {passengers.map((passenger, index) => (
            <div key={index} className={styles.summaryRow}>
              ...
              <Text>{formatPrice(selectedTrip.price * (1 - (passenger.discount || 0) / 100))}</Text>
            </div>
          ))}
