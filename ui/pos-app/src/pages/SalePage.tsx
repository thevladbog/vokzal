  ...
  const handleSell = async () => {
    ...
    try {
      ...
      await posService.printTicket({
        ...
        date: format(new Date(selectedTrip.departure_datetime), 'dd.MM.yyyy', { locale: ru }),
        time: format(new Date(selectedTrip.departure_datetime), 'HH:mm', { locale: ru }),
      ...
      <div className={styles.leftPanel}>
        ...
        <Card style={{ flex: 1, overflow: 'auto' }}>
          ...
          {trips?.map((trip) => (
            <Card
              key={trip.id}
              className={`${styles.tripCard} ${selectedTrip?.id === trip.id ? styles.selectedTrip : ''}`}
              onClick={() => setSelectedTrip(trip)}
            >
              <Text weight="bold" block>{trip.route_name}</Text>
              <Text block>
                {format(new Date(trip.departure_datetime), 'dd.MM.yyyy HH:mm', { locale: ru })}
              </Text>
              <Text block>
          ...
          {(!trips || trips.length === 0) && (
            <Text>Нет доступных рейсов</Text>
          )}
        </Card>
      </div>
          ...
          {selectedTrip ? (
            <div className={styles.saleForm}>
              <Card style={{ padding: '16px', backgroundColor: tokens.colorNeutralBackground3 }}>
                <Text weight="bold" block>{selectedTrip.route_name}</Text>
                <Text block>
                  {format(new Date(selectedTrip.departure_datetime), 'dd.MM.yyyy HH:mm', { locale: ru })}
                </Text>
