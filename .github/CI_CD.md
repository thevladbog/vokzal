# CI/CD Pipeline Documentation

Автоматизированная система непрерывной интеграции и развертывания для Вокзал.ТЕХ.

## 📋 Обзор

Проект использует **GitHub Actions** для автоматизации всех процессов разработки, тестирования и развертывания.

## 🔄 Рабочие процессы (Workflows)

### 1. Services CI (`services-ci.yml`)

**Триггеры:**
- Push в ветки `main`, `develop`
- Pull Request в `main`, `develop`
- Изменения в директории `services/**`

**Шаги:**
1. **Обнаружение изменений** — определяет, какие сервисы были изменены
2. **Линтинг и тестирование:**
   - Установка Go 1.23
   - Запуск `golangci-lint`
   - Запуск unit-тестов с coverage
   - Загрузка покрытия в Codecov
3. **Сборка Docker образов:**
   - Сборка для измененных сервисов
   - Push в Docker Hub (только для main/develop)
   - Кэширование слоев через GitHub Actions Cache
4. **Сканирование безопасности:**
   - Trivy для поиска уязвимостей
   - Загрузка результатов в GitHub Security

**Матрица сервисов:**
- auth, schedule, ticket, fiscal, payment
- board, notify, audit, document, geo

### 2. UI Applications CI (`ui-ci.yml`)

**Триггеры:**
- Push в ветки `main`, `develop`
- Pull Request в `main`, `develop`
- Изменения в директории `ui/**`

**Шаги:**
1. **Обнаружение изменений** — определяет, какие UI приложения изменились
2. **Линтинг и тестирование:**
   - Установка Node.js 20
   - ESLint проверка
   - TypeScript type-check
   - Jest тесты с coverage
3. **Сборка:**
   - Сборка production build
   - Загрузка артефактов
4. **Docker образы:**
   - Сборка и push (только для main/develop)

**Матрица приложений:**
- admin-panel, board-display
- passenger-portal, controller-app

### 3. Agent CI (`agent-ci.yml`)

**Триггеры:**
- Push в ветки `main`, `develop`
- Pull Request в `main`, `develop`
- Изменения в директории `agent/**`

**Шаги:**
1. **Линтинг и тестирование**
2. **Кросс-платформенная сборка:**
   - Linux (amd64)
   - Windows (amd64)
   - macOS (amd64, arm64)
3. **Release:**
   - Создание GitHub Release при тегах `v*`
   - Загрузка всех бинарников

### 4. Load Tests (`load-tests.yml`)

**Триггеры:**
- Расписание (nightly at 2 AM UTC)
- Manual dispatch

**Шаги:**
1. **Load тесты (k6):**
   - Тестирование производительности (auth, search-trips)
   - Загрузка результатов в артефакты

### 5. Deploy (`deploy.yml`)

**Триггеры:**
- Push в `main`
- Теги `v*.*.*`
- Manual dispatch с выбором окружения

**Шаги:**
1. **Развертывание сервисов:**
   - Настройка kubectl и Helm
   - Apply Kubernetes манифестов
   - Rollout статус
   - Запуск миграций БД
2. **Развертывание UI:**
   - Сборка приложений
   - Загрузка в S3
   - Инвалидация CloudFront
3. **Уведомления:**
   - Отправка в Telegram

### 6. Code Quality (`code-quality.yml`)

**Триггеры:**
- Push, Pull Request
- Еженедельно (воскресенье)

**Шаги:**
1. **CodeQL анализ** — JavaScript и Go
2. **Dependency Review** — проверка зависимостей в PR
3. **SonarCloud** — качество кода
4. **Markdown Lint** — проверка документации

### 7. Documentation (`docs.yml`)

**Триггеры:**
- Push в `main` (изменения в `docs/**`)
- Manual dispatch

**Шаги:**
1. **Сборка документации:**
   - VitePress build
   - Deploy в GitHub Pages
2. **Генерация OpenAPI:**
   - Swag для всех Go сервисов
   - Загрузка спецификаций
3. **Обновление README:**
   - Автоматическое обновление badges

### 8. Release (`release.yml`)

**Триггеры:**
- Теги `v*.*.*`
- Manual dispatch

**Шаги:**
1. **Создание релиза:**
   - Генерация changelog
   - Создание GitHub Release
2. **Сборка и публикация:**
   - Multi-arch Docker образы (amd64, arm64)
   - Публикация в Docker Hub
3. **Уведомления:**
   - Telegram о новом релизе

## 🔐 Секреты (GitHub Secrets)

### Docker Hub
- `DOCKER_USERNAME` — имя пользователя
- `DOCKER_PASSWORD` — токен доступа

### Kubernetes
- `KUBECONFIG` — base64 закодированный kubeconfig

### AWS (для UI deployment)
- `AWS_ACCESS_KEY_ID`
- `AWS_SECRET_ACCESS_KEY`
- `AWS_REGION`
- `ADMIN_PANEL_BUCKET`
- `ADMIN_PANEL_CF_ID`
- `PASSENGER_PORTAL_BUCKET`
- `PASSENGER_PORTAL_CF_ID`
- `BOARD_DISPLAY_BUCKET`
- `BOARD_DISPLAY_CF_ID`

### Database
- `DATABASE_URL` — connection string для миграций

### Notifications
- `TELEGRAM_CHAT_ID` — ID чата для уведомлений
- `TELEGRAM_BOT_TOKEN` — токен бота

### Code Quality
- `CODECOV_TOKEN` — токен для загрузки coverage
- `SONAR_TOKEN` — токен для SonarCloud

## 📊 Покрытие тестами

Codecov интегрирован для отслеживания покрытия кода:
- Минимальное покрытие: 70%
- Отчеты генерируются для каждого PR
- Флаги для разделения по сервисам/приложениям

## 🚀 Процесс релиза

### Автоматический релиз

1. Создайте тег:
```bash
git tag v1.0.0
git push origin v1.0.0
```

2. GitHub Actions автоматически:
   - Соберет все сервисы
   - Создаст Docker образы
   - Опубликует GitHub Release
   - Отправит уведомление в Telegram

### Ручной деплой

1. Перейдите в Actions → Deploy
2. Нажмите "Run workflow"
3. Выберите окружение (staging/production)
4. Запустите workflow

## 🏷️ Тегирование Docker образов

Образы тегируются по схеме:
- `latest` — последний коммит в main
- `main-<sha>` — коммит в main
- `develop-<sha>` — коммит в develop
- `v1.0.0` — релизная версия
- `1.0` — major.minor версия

## 📈 Мониторинг CI/CD

### GitHub Actions Dashboard
```
https://github.com/vokzal-tech/vokzal/actions
```

### Codecov Dashboard
```
https://codecov.io/gh/vokzal-tech/vokzal
```

### SonarCloud Dashboard
```
https://sonarcloud.io/project/overview?id=vokzal-tech_vokzal
```

## 🔧 Локальная разработка

### Запуск линтеров локально

**Go сервисы:**
```bash
cd services/auth
golangci-lint run
go test -v ./...
```

**UI приложения:**
```bash
cd ui/admin-panel
npm run lint
npm run type-check
npm test
```

### Pre-commit hooks

Установите pre-commit hooks:
```bash
# Install pre-commit
pip install pre-commit

# Install hooks
pre-commit install

# Run manually
pre-commit run --all-files
```

## 🐛 Устранение проблем

### Failed builds

1. Проверьте логи в GitHub Actions
2. Убедитесь, что все тесты проходят локально
3. Проверьте, что Docker образ собирается:
```bash
cd services/auth
docker build -t test .
```

### Failed deployments

1. Проверьте kubeconfig
2. Убедитесь, что Kubernetes кластер доступен
3. Проверьте статус pods:
```bash
kubectl get pods -n vokzal
kubectl logs -f deployment/auth-service -n vokzal
```

### Секреты не работают

1. Убедитесь, что все секреты добавлены в GitHub
2. Проверьте, что секреты не содержат лишних пробелов
3. Для base64 секретов (например, KUBECONFIG):
```bash
cat kubeconfig | base64 | pbcopy
```

## 📝 Best Practices

1. **Всегда создавайте PR** — не коммитьте напрямую в main
2. **Пишите тесты** — минимум 70% coverage
3. **Проверяйте линтеры** — перед push
4. **Используйте semantic versioning** — для тегов
5. **Добавляйте описания в PR** — для генерации changelog

## 🔗 Полезные ссылки

- [GitHub Actions Documentation](https://docs.github.com/en/actions)
- [Docker Hub](https://hub.docker.com/u/vokzaltech)
- [Kubernetes Documentation](https://kubernetes.io/docs/)
- [Helm Charts](https://helm.sh/docs/)

---

© 2026 Вокзал.ТЕХ
